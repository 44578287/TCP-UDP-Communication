...
namespace TCP_UDP通信
{
    public class Server
    {
        ...
        /// 构造函数
        /// </summary>
        /// <param name="useTCP">TCP/UDP选择</param>
        /// <param name="port">端口号</param>
        public Server(bool useTCP, int port)
        {
            udpPort = port;
            if (useTCP) // 使用TCP
            {
        ...
        /// <returns>任务</returns>
        /// <exception cref="ArgumentException">未连线ID</exception>
        ...
        public async Task SendToAllClients(string message)
        {
            ...
            Logger.WriteInfor($"已向{tcpClients.Values.Count}个客户端发送TCP消息: {message}");
        }
